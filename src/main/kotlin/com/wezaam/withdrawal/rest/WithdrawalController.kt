...
@Api
@RestController
class WithdrawalController @Autowired constructor(
    ...
    fun create(request: javax.servlet.http.HttpServletRequest): ResponseEntity<*> {
        ...
        if (userId == null || paymentMethodId == null || amount == null || executeAt == null) {
            return ResponseEntity<Any>("Required params are missing", HttpStatus.BAD_REQUEST)
        }
        ...
        if (!context.getBean(PaymentMethodRepository::class.java).findById(paymentMethodId.toLong()).isPresent) {
            return ResponseEntity<Any>("Payment method not found", HttpStatus.NOT_FOUND)
        }
